...
namespace ospf
{
    ...
    public:
        ...
        constexpr Error(C code)
            : _code(code), _msg(magic_enum::enum_cast<C>(code)) {}
        constexpr explicit Error(C code, std::string&& msg)
            : _code(code), _msg(std::forward(msg)) {}
    ...
    public:
        ...
        constexpr explicit ExError(C code, std::string&& msg)
            : Error<C>(code, std::forward(msg)), _arg(std::nullopt) {}
        constexpr explicit ExError(C code, std::string&& msg, RRefType<T> arg)
            : Error<C>(code, std::forward(msg)), _arg(move(arg)) {}
